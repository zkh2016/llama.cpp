...
#int8 vit and skip
    vit_int8)
        #mmproj=/DATA/disk1/zkh/Lenovo_V2_5/mmproj-model-Q8_0_M.gguf
        mmproj=/DATA/disk1/zkh/project/lenovo/llama.cpp/mmproj-model-Q8_0_M.gguf
        set -x
    ...
    lora)
        set -x
        ./build/bin/minicpmv-cli -m ${model} --mmproj ${mmproj} -c 4096 --temp ${temperature} --top-p ${top_p} --top-k 100 --repeat-penalty 1.05 -p "Please extract information from the PPT image given you and provide a brief description." --image ${image} -ngl 100 --skip-layers 8 --seed ${seed} --lora ${lora_model} 
        ;;
    fp_lora)
        set -x
