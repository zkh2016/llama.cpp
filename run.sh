...
fp_model=/DATA/disk1/zhangkaihuo/3b_sft_4k_for_zkh/model/ggml-model-f16.gguf
...
#no skip 
    no_skip)
        set -x
        ./build/bin/minicpmv-cli -m ${model} --mmproj ${mmproj} -c 8192 --temp ${temperature} --top-p ${top_p} --top-k 100 --repeat-penalty 1.05 -p "Please extract information from the PPT image given you and provide a brief description." --image ${image} --seed ${seed}
        ;;
    fp_no_skip)
        #set -x
...
#int8 vit and skip
    ...
    lora)
        set -x
        ./build/bin/minicpmv-cli -m ${model} --mmproj ${mmproj} -c 4096 --temp ${temperature} --top-p ${top_p} --top-k 100 --repeat-penalty 1.05 -p "Please extract information from the PPT image given you and provide a brief description." --image ${image} -ngl 100  --seed ${seed} --lora ${lora_model} -t 1 
        ;;
    fp_lora)
        set -x
