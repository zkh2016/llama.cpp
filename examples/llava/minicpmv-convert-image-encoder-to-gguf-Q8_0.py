...
for name, data in state_dict.items():
    ...
    if ".weight" in name and ("attn" in name or "ffn" in name):
        ...
        if 'ffn_down' in name:
            # in llama.cpp's clip, ffn_down is up_proj...
            new_out_channels = roundup(out_channels, group_size)
            qs = np.concatenate((qs, np.zeros([new_out_channels - out_channels, in_channels])), axis=0)
            scale = np.concatenate((scale, np.ones([new_out_channels - out_channels, 1]).astype(np.float32)), axis=0)
            out_channels = new_out_channels
            #print("new_out_channels = ", new_out_channels)
        elif 'ffn_up' in name:
            # in llama.cpp's clip, ffn_up is down_proj...
            new_in_channels = roundup(in_channels, group_size)
            qs = np.concatenate((qs, np.zeros([out_channels, new_in_channels - in_channels])), axis=1)
            in_channels = new_in_channels
            #print("new_in_channels = ", new_in_channels)
