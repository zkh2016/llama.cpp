...
ap.add_argument("--gptq", help="Path to GPTQ model directory cloned from HF Hub", required=True)
...
fname_out = os.path.join(output_dir, f"{fname_middle}model-Q8_0_M.gguf")
...
if has_minicpmv_projector:
    ...
    ftype_cur = 0
    name_map = {
    "resampler.attn.q.weight": "resampler.attn.q_proj.weight",
    "resampler.attn.q.bias": "resampler.attn.q_proj.bias",
    "resampler.attn.k.weight": "resampler.attn.k_proj.weight",
    "resampler.attn.k.bias": "resampler.attn.k_proj.bias",
    "resampler.attn.v.weight": "resampler.attn.v_proj.weight",
    "resampler.attn.v.bias": "resampler.attn.v_proj.bias",
    "resampler.attn.out.weight": "resampler.attn.out_proj.weight",
    "resampler.attn.out.bias": "resampler.attn.out_proj.bias",
    }
    for name, data in projector.items():
        name = get_tensor_name(name)
        data = data.squeeze().numpy()
        print('tensor name', name)
        #old_name = "resampler." + name#[len("vision_model."):]
        old_name = name if name not in name_map else name_map[name]
        ...
        if False:
            ...
            out_channels, in_channels = weight_encoding.weight.shape
            #print('tensor', data.shape)
            #print('encoding', weight_encoding.weight.shape)
            assert out_channels == weight_encoding.weight.shape[0]
            assert in_channels == weight_encoding.weight.shape[1]

            """
            new_data = data.reshape(-1, group_size)
            scale = np.abs(new_data.max(axis=1)) / (2**(8-1) -1)
            #scale = scale.reshape(-1, 1)
            qs = (new_data / scale[:, None]).round()
            qs[scale == 0] = 0
            """
            qs = weight_encoding.weight.numpy()
            scale = weight_encoding.scales.numpy()
            zeros = weight_encoding.zeros.numpy()
            #qs = qs - zeros
            qs = qs - np.repeat(zeros, group_size, axis=1)
            
            assert in_channels % group_size == 0
            assert out_channels % group_size == 0
            print(out_channels, in_channels)
            print("qs shape", qs.shape, "scale shape", scale.shape, "zeros shape", zeros.shape)
            #print(qs, qs.shape)
            #print(scale, scale.shape)
            #print(zeros, zeros.shape)
            #scale = scale.repeat(in_channels // group_size, axis=1).reshape(-1, 1)
            scale = scale.reshape(-1, 1)
            print("scale shape", scale.shape)
            qs = qs.reshape(-1, group_size)
            print("qs shape", qs.shape)
            d = scale.astype(np.float16).view(np.uint8)
            print('d', d.shape)
            new_data = np.concatenate((d, qs.astype(np.int8).view(np.uint8)), axis=1)
            #print("new_data.shape", new_data.shape, "qs.shape", qs.shape)
            new_data = new_data.reshape(out_channels, -1)
            #print(new_data, new_data.shape, new_data.dtype)
            print("new_data shape", new_data.shape)
            #print(data)
            print(f"{name} - Q8_0 - shape = {new_data.shape} {new_data.dtype}, offset={fout.offset_tensor}, nbytes={new_data.nbytes}")
            fout.add_tensor(name, new_data, raw_dtype=GGMLQuantizationType.Q8_0)
        else:
...
for name, data in state_dict.items():
    ...
    old_name = "vpm." + name[len("vision_model."):]
    ...
    if ".weight" in name and ("attn" in name or "ffn" in name):
        ...
        #qs = qs - zeros
        qs = qs - np.repeat(zeros, group_size, axis=1)
        #print("out_channels = ", out_channels, "in_channels = ", in_channels)
        ...
        print(f"{name} - Q8_0 - shape = {new_data.shape} {new_data.dtype}, offset={fout.offset_tensor}, nbytes={new_data.nbytes}")
        fout.add_tensor(name, new_data, raw_dtype=GGMLQuantizationType.Q8_0)
    elif 'ffn_down.bias' in name:
        group_size = 32
        out_channels, = data.shape
        new_out_channels = roundup(out_channels, group_size)
        data = np.concatenate((data, np.zeros([new_out_channels - out_channels]).astype(np.float32)), axis=0)
        #print(new_out_channels, out_channels, new_out_channels - out_channels)
        print(f"{name} - {ftype_str[ftype_cur]} - shape = {data.shape}, offset={fout.offset_tensor}")
        fout.add_tensor(name, data)
    else:
        print(f"{name} - {ftype_str[ftype_cur]} - shape = {data.shape}, offset={fout.offset_tensor}")
        fout.add_tensor(name, data)
