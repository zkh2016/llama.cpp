...
struct vpm {
    ...
    struct ggml_tensor *proj_bias;
    struct ggml_tensor *proj_weight;
    struct ggml_tensor *pos_embed; //(1, embed_len, embed_dim)

    std::vector<clip_layer> blocks;
    ...
    struct ggml_tensor *kv_proj_weight;

    struct ggml_tensor *ln_kv_bias;
    struct ggml_tensor *ln_kv_weight;

    struct ggml_tensor *ln_post_bias;
    struct ggml_tensor *ln_post_weight;

    struct ggml_tensor *ln_q_bias;
    struct ggml_tensor *ln_q_weight;
    struct ggml_tensor *resampler_pos_embed;
    struct ggml_tensor *resampler_proj;
    struct ggml_tensor *resampler_query;
};
...
static ggml_cgraph * clip_image_build_graph(clip_ctx * ctx, const clip_image_f32_batch * imgs) {
    ...
    const auto &model = ctx->vpm_model;
...
struct clip_ctx * clip_model_load(const char * fname, const int verbosity = 1) {
    ...
    if (new_clip->has_vision_encoder) {
        ...
        hparams.rms_norm_eps            = get_f32(ctx, format(KEY_LAYER_NORM_EPS, "vision"));
