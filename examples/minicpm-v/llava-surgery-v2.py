...
def clean_vision_tower_from_checkpoint(checkpoint_path):
    ...
    clip_tensors = [k for k, v in checkpoint.items() if k.startswith("vpm.blocks")]
...
def proj_criteria(checkpoint):
    return any(k.startswith("resampler.") for k in checkpoint.keys())
...
if projector_checkpoint_path is not None:
    ...
    mm_tensors = [k for k, v in last_checkpoint.items() if k.startswith("resampler.")]
