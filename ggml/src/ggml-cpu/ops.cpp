...
void ggml_compute_forward_sum_first_dim(
    ...
    int n = src->ne[3] * src->ne[1] * src->ne[0];
    for(int it = ith; it < n; it += nth){
        int k = it % src->ne[0];
        int j = it / src->ne[0] % src->ne[1];
        int i = it / (src->ne[0] * src->ne[1]);
        float sum = 0;
...
void ggml_compute_forward_sparse_soft_max(
        const ggml_compute_params * params,
              ggml_tensor * dst) {
...
void ggml_compute_forward_compress_k(
    ...
    for(int i = ith; i < ggml_nelements(dst); i+= nth){
        ...
        int oy = (i / px) % py;
        int oz = i / (px * py);
        c_dplane = c_dplane + oy * dst->nb[1] + oz * dst->nb[2];
        {
            float *out;
            if(ox < 64){
                out = ((float*)c_dplane) + ox * 2;
            }else{
                out = ((float*)c_dplane) + (ox - 64) * 2 + 1;
            }
            switch (op) {
                case GGML_OP_POOL_AVG:     *out = 0;        break;
                case GGML_OP_POOL_MAX:     *out = -FLT_MAX; break;
            ...
            for (int ky = 0; ky < k1; ++ky) {
                if (iy + ky < 0 || iy + ky >= src->ne[1]) continue;
                const void * srow = (const void *)(cdata + src->nb[1] * (iy + ky) + oz * src->nb[2]);
                for (int kx = 0; kx < k0; ++kx) {
                    int j = ix + kx;
                    if (j < 0 || j >= src->ne[0]) continue;
                    j = j < 64 ? j * 2 : (j-64)*2+1; 
...
static void ggml_compute_forward_block_sparse_attn_ext_f16(
        const ggml_compute_params * params,
        const ggml_tensor * q,
        const ggml_tensor * k,
        const ggml_tensor * v,
    ...
    memcpy(&max_bias,      (float *) dst->op_params + 1, sizeof(float));
    memcpy(&logit_softcap, (float *) dst->op_params + 2, sizeof(float));
    memcpy(&topk,         (int*) dst->op_params + 3, sizeof(int));
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        for(int topi = 0; topi < topk; ++topi) {
            ...
            for(int ic = start; ic >= 0 && ic < end && ic < nek1 && (neq1 == 1 || iq1 >= ic); ++ic){
                f(ic);
            }
        }
...
void ggml_compute_forward_block_sparse_attn_ext(
    ...
    switch (dst->op_params[3]) {
        ...
        case GGML_PREC_F32:
            {
                // uses F32 accumulators
                ggml_compute_forward_block_sparse_attn_ext_f16(params, q, k, v, topk_idx, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
void ggml_compute_forward_transform_score(
    ...
    for(int i = ith; i < n; i+= nth){
        ...
        float * const drow = dplane + oy * px + oz * px * py;
        float * const out =  drow + ox;
        int q_id = (oy + n_tokens - py) / block_size;
        if(q_id - local_blocks < ox){
            *out = -INFINITY;
            continue;
        }else if(ox < init_blocks){
            *out = INFINITY;
            continue;
        }
        switch (op) {
